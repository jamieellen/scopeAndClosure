...
/*Task 1: counterMaker()
  ...
  1. What is the difference between counter1 and counter2?
          counter1 is a variable assigned to execute a multi-scoped function and keep hold of the count.
          Whereas counter2 is a function in and of itself. 
  
  2. Which of the two uses a closure? How can you tell?
        counter1 uses a closure as it has a function nested inside of another function, accessing variables from the outer function.
  
  3. In what scenario would the counter1 code be preferable? In what scenario would 
     counter2 be better?  
        counter1 would be preferable in the case of keeping track of multiple teams scores using the same function
        buit assigning it to different variables.
        counter2 would be prefered in the case of nneding to track one a single specified itme in the same increment over time.
*/
...
function scoreboard(inningScoreCB, inningCB, numInnings) {
  ...
  for(let i = 0; i < numInnings; i++) {
    const currentInning = inningScoreCB(inningsCB);
    homeScore += currentInning.Home
    awayScore += currentInning.Away
    scoreByInnings.push(`Innings ${i + 1}: Away ${currentInning.Away} - Home ${currentInning.Home}`)
  }
  if(homeScore === awayScore) {
    scoreByInnings.push(`This game will require extra innings: Away ${currentInning.Away} - Home ${currentInning.Home}`);
  } else {
